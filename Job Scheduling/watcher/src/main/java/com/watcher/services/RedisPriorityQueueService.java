...
//    }
    ...
    public void addToQueue(Job job, LocalDateTime executionTime) {
        ...
        try {
            ...
            String jobKey = job.getId() + ":" + executionTime.toString();

            // Score = epoch millis
            double score = executionTime.toInstant(ZoneOffset.UTC).toEpochMilli();

            // Use ZADD NX → add only if not exists
            Boolean added = redisTemplate.opsForZSet().addIfAbsent(REDIS_KEY, jobKey, score);

            if (Boolean.TRUE.equals(added)) {
                // Store job details separately in HASH
                String hashKey = "job:data:" + jobKey;
                String json = objectMapper.writeValueAsString(Map.of(
                        "job", job,
                        "time", executionTime.toString()
                ));
                redisTemplate.opsForHash().put(hashKey, "payload", json);

                log.info("✅ Added job {} to Redis at {}", job.getJobSeqId(), executionTime);
            } else {
                log.info("⚠️ Job {} at {} already exists, skipping", job.getJobSeqId(), executionTime);
            }

        } catch (Exception e) {
            log.error("❌ Failed to add job {}: {}", job.getJobSeqId(), e.getMessage(), e);
        }
    }
