...
////////    private static String logFile; // dynamic per execution
////////
...
////////        // Determine job name from Jenkinsfile path
...
////////        logFile = "logs/" + jobName + "_" + dtf.format(LocalDateTime.now()) + ".log";
////////
...
////////            }
////////            for (Map.Entry<String, String> entry : pipelineEnv.entrySet()) {
////////                cmd = cmd.replace("${" + entry.getKey() + "}", entry.getValue());
////////            }
////////
////////            String stageName = "Stage-" + stageNum;
////////            logStageStart(stageName);
////////            boolean success = runCommand(stageName, cmd);
////////            if (!success) {
////////                logError("Execution stopped due to error in stage: " + stageName);
////////                break;
////////            }
////////            stageNum++;
////////        }
...
////////        boolean inEnvBlock = false;
////////        for (String line : lines) {
////////            line = line.trim();
////////            if (line.startsWith("environment {")) inEnvBlock = true;
////////            else if (line.startsWith("}")) inEnvBlock = false;
////////            else if (inEnvBlock && line.contains("=")) {
...
////////    }
////////
////////    private void logStageStart(String stageName) throws IOException {
////////        logToFile("\n==================================================");
////////        logToFile("[" + LocalDateTime.now() + "] Starting: " + stageName);
////////        logToFile("==================================================");
////////    }
////////
////////    private void logStageEnd(String stageName, int exitCode) throws IOException {
////////        logToFile("--------------------------------------------------");
////////        logToFile("[" + LocalDateTime.now() + "] Finished: " + stageName + " (exit code " + exitCode + ")");
////////        logToFile("--------------------------------------------------\n");
////////    }
////////
...
////////    }
////////
////////    // Run command with dynamic interpreter logic (cmd/bash/python/etc.)
////////    private boolean runCommand(String stageName, String command) throws IOException, InterruptedException {
...
////////
////////        if (isWindows) {
////////            if (interpreter.equalsIgnoreCase("powershell")) builder.command("powershell.exe", "-Command", args);
////////            else if (interpreter.equalsIgnoreCase("cmd")) builder.command("cmd.exe", "/c", args);
...
////////                logToFile("[" + LocalDateTime.now() + "] " + line);
////////            }
////////        }
////////
////////        int exitCode = process.waitFor();
////////        logStageEnd(stageName, exitCode);
////////        return exitCode == 0;
////////    }
////////
...
////////            executor.parseAndRun("D:/lap/java/dsa/jenkins/Jenkinsfile");
////////        } catch (Exception e) {
////////            e.printStackTrace();
////////        }
////////    }
...
//////    private static final Map<String, String> envMap = EnvMapProvider.getEnvMap();
//////
//////    // Executor service for parallel stages
...
//////
//////        // Execute sequentially for now (can expand for parallel/matrix)
//////        int stageNum = 1;
...
//////            logStageStart(stageName);
//////
//////            // Submit command for execution (parallel execution possible)
//////            String finalCmd = cmd;
//////            Future<Boolean> future = executorService.submit(() -> runCommand(stageName, finalCmd));
//////            futures.add(future);
//////
//////            stageNum++;
//////        }
...
//////            if (!future.get()) {
//////                logError("Pipeline stopped due to failure.");
//////                break;
//////            }
//////        }
//////
...
//////            else if (line.startsWith("}")) inBlock = false;
//////            else if (inBlock) {
//////                // Example: maven 'Maven 3.9'
//////                String[] parts = line.split("'", 2);
//////                if (parts.length == 2) {
//////                    String key = parts[0].trim(); // maven / jdk
...
//////        try {
//////            JenkinsfileExecutor executor = new JenkinsfileExecutor();
//////            executor.parseAndRun("D:/lap/java/dsa/jenkins");
//////        } catch (Exception e) {
//////            e.printStackTrace();
//////        }
//////    }
...
////                .collect(Collectors.toList());
////
////        // Define matrix axes (can be read from Jenkinsfile in the future)
////        List<String> osAxis = Arrays.asList("windows", "linux");
////        List<String> jdkAxis = Arrays.asList("8", "11");
////
////        // Generate all combinations
////        List<Map<String, String>> matrixCombinations = new ArrayList<>();
////        for (String os : osAxis) {
////            for (String jdk : jdkAxis) {
////                Map<String, String> combo = new HashMap<>();
////                combo.put("OS", os);
////                combo.put("JDK", jdk);
////                combo.putAll(pipelineEnv);
////                matrixCombinations.add(combo);
////            }
////        }
////
////        // Run matrix combinations in parallel
////        List<Future<Boolean>> futures = new ArrayList<>();
////        for (Map<String, String> combo : matrixCombinations) {
////            futures.add(executorService.submit(() -> runMatrixCombination(jobName, commands, combo)));
////        }
////
////        // Wait for completion
////        for (Future<Boolean> future : futures) {
...
////        String comboName = "OS=" + env.get("OS") + "_JDK=" + env.get("JDK");
...
//            // No matrix → run a single default combination
